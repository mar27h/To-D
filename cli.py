...
import functions #ten sposób zapisywania jest lepszy - bo wiadomo skąd się biorą funkcje
import time
...
while True:
    ...
    if user_action.startswith("add"):#if 'add' in user_action or 'new' in user_action:
        ...
        todos=functions.get_todos() #mozna podawać wprost parametr do którego przypisujemy argument (filepath=f"../../Files/todos.txt") albo nic nie pisać i zostanie
                        # uzyty defaultowy parametr

        todos.append(todo)

        functions.write_todos(todos,f"../../Files/todos.txt")


    elif user_action.startswith("show"):#'show'in user_action:

        todos=functions.get_todos(f"../../Files/todos.txt")
        ...
        try:
            ...
            todos=functions.get_todos()#nie musimy podawać żadnego argumentu

            new_todo =input("Enter new todo: ")
            todos[number]=new_todo+"\n"

            functions.write_todos(todos)#nie musimy podawać drugiego argumentu, który nie ma wartości defaultowej
